@Code
    ViewData("Title") = "Remove a subject from a course"
End Code
...
<div class="row" style="padding-top:100px;">
    ...
    <div class="col-lg-9">
        @Using (Html.BeginForm())
            ...
            @<h3>REMOVE A SUBJECT FROM A COURSE</h3>
            @<hr style="background-color:cadetblue;height:2px;" />
            @<table border="0" cellpadding="5" cellspacing="5">
                ...
                @if Model.lstTaughtCoursesForDDL.Count() > 0 Then                    @<tr>
            <td colspan="2">
                @Html.Label("You are teaching these courses currently, please select the one you want to remove", htmlAttributes:=New With {.class = "control-label col-md-2", .style = "width:100%;"})
                    ...
                    @if Model.lstTaughtSubjectsForDDL.Count() > 0 Then
                        @<tr>
                        <td colspan="2">
                            @Html.Label("These subjects are currently taught in this course. Please select the one you want to remove", htmlAttributes:=New With {.class = "control-label col-md-2", .style = "width:100%;"})
                        </td>
                    </tr>
                    @<tr>
                        <td colspan="2">
                            @Html.DropDownListFor(Function(model) model.SelectedSubject, Model.lstTaughtSubjectsForDDL, htmlAttributes:=New With {.class = "control-label col-md-2", .style = "width:100%;"})
