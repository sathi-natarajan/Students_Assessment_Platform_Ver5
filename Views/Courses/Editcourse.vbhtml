...
<script>
    ...
    function LoadCourseDetails() {
        var serviceURL = '/Courses/GetCourseDetails'; //This should not be inside AJAX call itself
    ...
    function successFunc(data, status) {
        ...
        var strQuestionInfo = "<strong>Course ID:</strong>" + data.CourseID + "<br/>";
        strQuestionInfo += "<strong>Classname:</strong>" + data.Classname + "<br/>";
        strQuestionInfo += "<strong>Course Description:</strong>" + data.Description + "<br/>";
        strQuestionInfo += "<strong>Start date:</strong>" + data.Startdate + "<br/>";
        strQuestionInfo += "<strong>End date:</strong>" + data.Enddate + "<br/>";
        strQuestionInfo += "<strong>Right now, just displays Course info.  Later, will implement editing these things</strong>";
    ...
    function errorFunc() {
        ...
        $("#dlgQuestionInfo").html("<p>An error occured when trying to fetch information on the selected COURSE</p>")
        $("#dlgQuestionInfo").dialog({
            resizable: false,
            title: "ERROR",
            height: "auto",
        ...
        @Using (Html.BeginForm())
            ...
            @<table border="0" cellpadding="2" cellspacing="2">
                ...
                @if Model.lstTaughtCoursesForDDL.Count()>0
                    @<tr>
                        <td colspan="2">
                            @Html.Label("You are teaching these courses currently, please select the one you want to edit", htmlAttributes:=New With {.class = "control-label col-md-2", .style = "width:100%;"})
                        </td>
                    </tr>
                    @<tr>
                        <td colspan="2">
                            @Html.DropDownListFor(Function(model) model.SelectedCourse, Model.lstTaughtCoursesForDDL, htmlAttributes:=New With {.class = "control-label col-md-2", .style = "width:100%;", .onchange = "LoadCourseDetails();"})
                            @Html.ValidationMessageFor(Function(model) model.SelectedCourse, "", New With {.class = "text-danger"})
                        </td>
                    </tr>
                    @<tr>
                        <td colspan="2" style="text-align:center;">
                            <input type="submit" value="UPDATE COURSE" Class="btn btn-success clsbutton-round" style="background:cadetblue;" />
