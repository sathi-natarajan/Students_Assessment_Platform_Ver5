        ...
        @Using (Html.BeginForm())
            ...
            @<table border="0" cellpadding="2" cellspacing="2">
                ...
                @if Model.TestsList.Count()>0
                    @<tr>
                        <td colspan="2">
                            @Html.Label("You have created these tests, please select the one you want to remove", htmlAttributes:=New With {.class = "control-label col-md-2", .style = "width:100%;"})
                        </td>
                    </tr>
                    @<tr>
                        <td colspan="2">
                            @Html.DropDownListFor(Function(model) model.SelectedTest, Model.TestsList, htmlAttributes:=New With {.class = "control-label col-md-2", .style = "width:100%;"})
                            @Html.ValidationMessageFor(Function(model) model.SelectedTest, "", New With {.class = "text-danger"})
                        </td>
                    </tr>
                    @<tr>
                        <td colspan="2" style="text-align:center;">
                            <input type="submit" value="REMOVE TEST" Class="btn btn-success clsbutton-round" style="background:cadetblue;" />
                ...
                Else
                    @<tr>
                        <td colspan="2" style="text-align:center;">
                            You have not created any tests yet. Nothing to show for removal
                        </td>
                    </tr>
                    @<tr>
                        <td colspan="2" style="text-align:center;">
